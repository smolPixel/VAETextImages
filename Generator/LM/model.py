...
class LM_model(nn.Module):
    def __init__(self, argdict, decoder):

        super().__init__()
        self.argdict=argdict
        self.tensor = torch.cuda.FloatTensor if torch.cuda.is_available() else torch.Tensor
        self.decoder=decoder
        self.latent_size=self.argdict['latent_size']

    def forward(self, batch):

        input_sequence=batch['input'].to('cuda')
        hidden=torch.zeros((1, batch['input'].shape[0], self.latent_size)).to('cuda')
        logp = self.decoder(input_sequence, hidden)
