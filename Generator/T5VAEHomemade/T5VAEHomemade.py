...
# )
from Generator.T5VAEHomemade.model import ModifiedT5ForConditionalGenerationHomemade
...
class T5VAEHomemade(LightningModule):
	def __init__(self, argdict, train, dev, test):
		super().__init__()
		self.config = T5Config.from_pretrained(argdict['base_model'], add_special_tokens=True)
		self.t5 = ModifiedT5ForConditionalGenerationHomemade.from_pretrained(
			argdict['base_model'],
			config=self.config,
			argdict=argdict
		)
	...
	def train_model(self):
		...
		self = T5VAEHomemade.load_from_checkpoint(
