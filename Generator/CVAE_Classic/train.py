...
from model import CVAE_Classic
...
def main(args):
    ...
    def loss_fn(logp, target, length, mean, logv, anneal_function, step, k, x0):
        ...
        target = target[:, :params['max_sequence_length']].contiguous().view(-1)
    ...
    for epoch in range(args.epochs):
        ...
        for split in splits:
            ...
            if split == 'valid':
                ...
                if not os.path.exists(os.path.join('CVAE_Classic/dumps', ts)):
                    os.makedirs('CVAE_Classic/dumps/'+ts)
                with open(os.path.join('CVAE_Classic/dumps/'+ts+'/valid_E%i.json' % epoch), 'w') as dump_file:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-nc', '--nb_classes', type=int, default=2)
    ...
    parser.add_argument('-log', '--logdir', type=str, default='CVAE_Classic/logs')
    parser.add_argument('-bin', '--save_model_path', type=str, default='CVAE_Classic/bin')
