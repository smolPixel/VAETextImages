...
class Discri_dataset(Dataset):
    def __init__(self, pos, neg, argdict):
        super().__init__()
...
class SeqGANInstructor:
    ...
    def train_gen_epoch(self, model, dataset, criterion, optimizer):
        ...
        for i, data in enumerate(data_loader):
            inp, target = torch.Tensor(data['input']).type(torch.LongTensor), torch.Tensor(data['target']).type(
                torch.LongTensor)
    ...
    def train_dis_epoch(self, model, pos_data, neg_data, criterion, optimizer):
        total_loss = 0
        total_acc = 0
        total_num = 0

        dataset=Discri_dataset(pos_data, neg_data, self.argdict)
    ...
    def train_discriminator(self, d_step, d_epoch, phase='MLE'):
        ...
        for step in range(d_step):
            ...
            neg_samples = self.gen.sample(self.argdict['samples_num'], 4 * self.argdict['batch_size'],
                                          start_letter=self.training_set.sos_idx)
